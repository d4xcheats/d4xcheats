...
public class Auth extends AsyncTask<String, Void, String> {
    ...
    protected void onPreExecute() {
        ...
        if (getDialog() != null) {
            getDialog().setMessage("Try to login...");
            getDialog().show();
        }
    }
    ...
    protected String doInBackground(String... strings) {
        ...
        try {
            ...
            data.put("uuid", getUniqueId(getActivity()));
    ...
    protected void onPostExecute(String s) {
        ...
        if(s == null || s.isEmpty()){
            Toast.makeText(activity,(Html.fromHtml("<font face='monospace'> <font color='#ff0000'>Server error!</font></font>")), Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        try {
            ...
            if (!verify(decData, ack.get("Sign").toString(), puk)) {
                Toast.makeText(activity, (Html.fromHtml("<font face='monospace'> <font color='#ff0000'>Data Salah!</font></font>")), Toast.LENGTH_SHORT).show();
                return;
            }
            JSONObject data = new JSONObject(decData);
                if (data.get("Status").toString().equals("Success")) {
                    Toast.makeText(activity, data.get("MessageString").toString(), Toast.LENGTH_SHORT).show();
                    Prefs prefs = Prefs.with(activity);
                    String EXPIRED = "EXPIRED";
                    prefs.write(EXPIRED, data.get("SubscriptionLeft").toString());
                    loginLoaded = true;
                    ALREADYLOGIN();
                    getActivity().startActivity(new Intent(activity, Class.forName(getActivity().MainAct())));
                    getActivity().finish();
                } else {
                    Toast.makeText(activity, data.get("MessageString").toString(), Toast.LENGTH_SHORT).show();
                }
